...
public class LoopToolTests {
    ...
    public @Test void methodGetCountOrGetIndex() throws Exception
    {
        LoopTool loop = new LoopTool();
        Iterator i = loop.watch(ARRAY);
        assertEquals(Integer.valueOf(0), loop.getCount());
        assertNull(loop.getIndex());
        i.next();
        assertEquals(Integer.valueOf(1), loop.getCount());
        assertEquals(Integer.valueOf(0), loop.getIndex());
        i.next();
        assertEquals(Integer.valueOf(2), loop.getCount());
        assertEquals(Integer.valueOf(1), loop.getIndex());
        i.next();
        assertEquals(Integer.valueOf(3), loop.getCount());
        assertEquals(Integer.valueOf(2), loop.getIndex());
        loop.pop();
    ...
    public @Test void methodGetCountOrGetIndex_String() throws Exception
    {
        LoopTool loop = new LoopTool();
        Iterator i = loop.watch(ARRAY, "i");
        assertEquals(Integer.valueOf(0), loop.getCount("i"));
        assertNull(loop.getIndex("i"));
        i.next();
        assertEquals(Integer.valueOf(1), loop.getCount("i"));
        assertEquals(Integer.valueOf(0), loop.getIndex("i"));
        Iterator j = loop.watch(ARRAY, "j");
        loop.skip(2);
        assertEquals(Integer.valueOf(2), loop.getCount("j"));
        assertEquals(Integer.valueOf(1), loop.getIndex("j"));
        assertEquals(Integer.valueOf(1), loop.getCount("i"));
        assertEquals(Integer.valueOf(0), loop.getIndex("i"));
        // check short syntax too
